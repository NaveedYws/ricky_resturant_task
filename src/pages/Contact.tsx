...
const Contact = () => {
  ...
  const inputClass = "w-full bg-background border border-border px-4 py-3 rounded font-body text-sm text-foreground placeholder:text-muted-foreground focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary transition-colors";
            ...
            <div>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  ...
                  <input type="number" placeholder="Number of Guests" value={form.guests} onChange={(e) => setForm({ ...form, guests: e.target.value })} min="1" className={inputClass} />
                  <input type="date" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} className={inputClass} />
                  <input type="time" value={form.time} onChange={(e) => setForm({ ...form, time: e.target.value })} className={inputClass} />
                </div>
                <textarea placeholder="Special Requests" value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} rows={4} className={`${inputClass} resize-none`} />
                <button type="submit" className="w-full bg-primary text-primary-foreground px-8 py-4 rounded font-body text-sm tracking-widest uppercase transition-all duration-300 hover:bg-gold-dark">
                  Submit Reservation
                </button>
              </form>
            </div>
            ...
            <div>
              ...
              <div className="rounded-lg overflow-hidden shadow-md h-[300px]">
                <iframe
                  ...
                  referrerPolicy="no-referrer-when-downgrade" title="Savoria Restaurant Location"
                />
              </div>
            </div>
          </div>
